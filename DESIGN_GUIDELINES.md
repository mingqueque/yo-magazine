...
- 필수 조건 충족 후 활성화되는 CTA는 비활성 상태에서 `--color-surface-soft` 배경과 `--color-text-subtle` 텍스트를 사용하고, 활성 상태에서는 `--color-brand-main` 배경과 흰색 텍스트를 사용한다.
...
- 댓글을 구분하는 얇은 디바이더는 `--color-comment-divider`를 사용하고 좌우 마진 없이 풀폭으로 노출한다. 댓글 카드 상하 패딩은 전역 `12px`로 둔다.
