...
- 기본 화면 좌우 여백은 `--space-16: 16px`이다. 특정 섹션만 즉흥적으로 다른 좌우 여백을 넣지 않는다.
...
- 프레임형 화면 전환 기준: `520px`
- 기본 화면 좌우 여백: `--space-16`
- 하단 탭을 고려한 화면 하단 여백: `82px`

## 전역 컴포넌트 규칙

- 보텀 모달시트는 OS 바텀시트 디자인을 따른다.
- 시트 상단에는 닫기 X 버튼을 두지 않고, OS식 그랩 핸들을 사용한다.
- 시트는 하단 안전영역을 반영하고, 상단 모서리 라운드와 딤 배경, 하단에서 올라오는 모션을 공통 `.modal-*` 스타일에서 관리한다.
- 보텀 모달시트 안의 옵션 버튼에는 쉐브론이나 별도 아이콘을 배치하지 않는다.
- 신고, 차단, 로그인 게이트처럼 보텀시트를 쓰는 화면은 개별 화면에서 별도 시트 디자인을 만들지 않는다.
...
- `--color-text-primary`: 주요 텍스트, `#212121`
- `--color-text-body`: 본문 텍스트, `#212121`
- `--color-text-secondary`: 보조 텍스트
- `--color-text-subtle`: 더 약한 보조 텍스트/chevron
- `--color-text-book`: 책 배너 보조 텍스트
- `--color-divider`: 모든 디바이더 컬러. `divider_6px`와 동일한 `--color-surface-soft`
- `--color-border`: 기본 경계선
- `--color-border-strong`: 뱃지 등 강조 경계선
- `--color-brand-main`: 메인 액션/선택 색
- `--color-brand-pressed`: 메인 액션 누름 상태
- `--color-danger`: 작성자 차단하기처럼 경고 의미가 담긴 버튼 레이블 색. 값은 `#BD0021`을 사용한다.
...
- `--font-label: 13px`: 보조 버튼, 알림 본문
- `--font-body: 14px`: 기본 본문/버튼
- `--font-card-title: 15px`: 게시글 카드 제목, 알림 제목, 글작성 섹터 타이틀
- `--font-cta: 15px`: CTA 종류 버튼 레이블
...
- 메인 타이틀 바로 아래 보조문구가 붙는 경우 두 요소 사이 간격은 `4px`를 사용한다.
...
- CTA 종류 버튼 레이블은 `--font-cta: 15px`와 `--font-weight-cta: 700`을 사용한다.
- 글작성 페이지의 `카테고리`, `제목`, `글` 섹터 타이틀은 `15px`, `700` 웨이트를 사용한다.
- 입력필드는 포커스/터치 상태에서 브랜드 빨간색 테두리나 outline을 표시하지 않고 기본 회색 테두리를 유지한다.
- 필수 조건 충족 후 활성화되는 CTA는 비활성 상태에서 보더라인 없이 `--color-surface-soft` 배경과 `--color-text-subtle` 텍스트를 사용하고, 활성 상태에서는 `--color-brand-main` 배경과 흰색 텍스트를 사용한다.
- 앱 전역의 기본 화면 좌우 여백은 `16px`이다.
- 섹션에 소속된 더보기 형태의 롱버튼 레이블은 홈탭 `요즘 연애` 섹션 더보기 버튼을 기준으로 `--font-label`, `400` 웨이트를 사용한다.
- 제목과 아이콘세트가 같은 로우에 있을 때 두 영역 사이 간격은 최소 `12px`을 확보한다. 제목은 1줄 말줄임 처리하고, 아이콘세트 영역을 침범하지 않도록 제목 컬럼에는 `min-width: 0`을 둔다.
- 게시글 상세 본문과 좋아요/댓글 액션 사이 간격은 `20px`로 둔다.
- 게시글 상세 댓글 섹션 헤더에는 정렬 레이블을 노출하지 않는다.
- 게시글 상세 댓글 리스트에서는 댓글과 댓글 사이의 얇은 디바이더 선을 노출하지 않는다.
- 댓글을 구분하는 얇은 디바이더는 `--color-comment-divider`를 사용하고 좌우 마진 없이 풀폭으로 노출한다. `--color-comment-divider`는 `--color-divider`와 같은 컬러를 사용한다. 댓글 카드 상하 패딩은 전역 `12px`로 둔다.
- 앱 내 두꺼운 디바이더는 `6px`를 사용하며, 이 스타일을 `divider_6px`라고 부른다. 1px 디바이더는 `divider_1px`라고 부른다. 모든 디바이더 컬러는 `divider_6px` 컬러와 통일한다.
- 홈탭 `요즘 연애` 섹션 내 글 아이템 구분선은 `divider_1px`를 적용한다.
- 앱 전역의 디바이더는 항상 화면 좌우를 가득 채운다. 콘텐츠가 화면 기본 좌우 패딩 안에 있더라도 디바이더 라인은 패딩 밖까지 확장한다.
...
- 마이페이지 첫 화면 프로필 카드에는 아바타와 로그인 방식 보조 문구를 노출하지 않고, `익명으로 활동 중` 문구와 이동 chevron만 표시한다. 로그인 방식은 계정 정보 화면에서만 보여준다.
